...
# ============================================================
# データ取得  ← 修正箇所: 全て DatetimeIndex で統一
# ============================================================
...
def fetch_us_close(start: str, end: str) -> pd.DataFrame:
    ...
    df.index = pd.to_datetime(df.index).normalize()   # DatetimeIndex
    return df[US_TICKERS]

def fetch_jp_ohlcv(start: str, end: str) -> dict:
    """JP ETF の OHLCV を {ticker: DataFrame(DatetimeIndex)} で返す"""
    result = {}
...
def verify_positions(prev_pos: dict, jp_ohlcv: dict, verify_date: date) -> dict:
    ...
    vts = to_ts(verify_date)   # date → Timestamp
...
# ============================================================
# メイン  ← 修正箇所: Timestamp で全ルックアップを統一
# ============================================================
...
def main():
    ...
    us_raw       = fetch_us_close(fs, fe)           # DatetimeIndex
    us_data_date = us_raw.index[-1].date()
    print(f"   米国データ最新日: {us_data_date}")

    # --- JP データ ---
    print("[3/7] 日本 TOPIX-17 ETF データ取得中...")
    jp_ohlcv = fetch_jp_ohlcv(fs, fe)              # {ticker: DataFrame(DatetimeIndex)}

    # Close DataFrame を構築 ← 修正: Series.rename でインデックスを揃えてから concat
    close_series_list = []
    for ticker, odf in jp_ohlcv.items():
        s = odf['Close'].rename(ticker)
        close_series_list.append(s)
    jp_close_raw = pd.concat(close_series_list, axis=1)  # DatetimeIndex × tickers

    # 利用可能 JP 日付（date オブジェクト）
    ...
    print(f"   JP データ利用可能: {len(avail_jp)} 日")
    ...
    us_latest_ret = us_ret_full.iloc[-1].values
    ...
    if prev_pos:
        try:
            ...
            prev_td = None
        if prev_td and prev_td in avail_jp:
            verify_date = prev_td
        else:
    ...
    signals_ranked = {
        name: {
            ...
            'code'    : n2t.get(name,'').replace('.T',''),
            'class'   : JP_TICKER_MAP[n2t[name]][1] if name in n2t else '',
    ...
    print(f"\n=== 完了 ===  LONG:{long_names}  SHORT:{short_names}")
    if verification:
        print(f"前回検証 R = {verification['strategy_return_pct']:+.4f}% {verification['win_loss']}")
