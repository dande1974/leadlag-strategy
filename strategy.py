...
def flatten_columns(df):
    """
    yfinance 新バージョンは MultiIndex columns を返すことがある。
    例: ('Close', '1617.T') → 'Close' にフラット化する。
    """
...
def fetch_us_close(start, end):
    """US ETF 終値を DataFrame(DatetimeIndex × tickers) で返す"""
    df = yf.download(US_TICKERS, start=start, end=end,
                     progress=False, auto_adjust=True)
    df = flatten_columns(df)
    close = df['Close'] if 'Close' in df.columns else df.iloc[:, 0]
    # US の場合は multiindex で Close が最上位になるケースもある
...
def fetch_jp_ohlcv(start, end):
    ...
    for ticker in JP_TICKER_MAP:
        try:
            ...
            if df.empty:
                continue
            df = flatten_columns(df)        # MultiIndex → flat
...
def build_jp_close_df(jp_ohlcv):
    """
    jp_ohlcv から Close の DataFrame を構築。
    .rename() を使わず .name = ticker で安全に設定。
    """
    series_list = []
    for ticker, odf in jp_ohlcv.items():
        s = odf['Close'].copy()
        s.name = ticker          # ← rename() は使わない
...
def verify_positions(prev_pos, jp_ohlcv, verify_date):
...
def main():
    ...
    if LAST_POS_JSON.exists():
        ...
        print(f"✅ 前回ポジション: LONG={prev_pos['long_positions']} SHORT={prev_pos['short_positions']}")

    # 1. JP 営業日
    print("[1/7] JP 営業日カレンダー構築中...")
    jp_days = build_jp_trading_days()
    print(f"   📅 {len(jp_days)} 営業日")
    ...
    jp_close_raw = build_jp_close_df(jp_ohlcv)   # ← 修正済み関数を使用
    ...
    print(f"[4/7] ウィンドウ終端: {window_end}  JP列数: {jp_ret_win.shape[1]}  長さ: {len(jp_ret_win)}")
    ...
    print(f"✅ シグナル計算完了  米国:{us_data_date}  JP目標:{target_date}")
